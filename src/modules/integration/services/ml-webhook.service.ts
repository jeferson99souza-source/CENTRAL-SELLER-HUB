...
export class MlWebhookService {
  ...
  async handleEvent(body: any, query: any) {
    ...
    if (!topic || !resource || !sellerId) {
      this.logger.debug(
        `Webhook sem dados essenciais: ${JSON.stringify(body)}`,
      );
      return;
    }
  ...
  private async handleMessage(
    ...
    const content = (
      typeof msg.text === 'string' ? msg.text : (msg.text?.plain ?? '')
    ).trim();
    ...
    this.logger.log(
      `Mensagem salva via webhook: externalId=${externalId} sender=${sender} buyer=${buyerName}`,
    );
  }
  ...
  private async handleQuestion(
    ...
    if (exists) {
      if (q.status === 'ANSWERED' && exists.status === 'unanswered') {
        ...
        this.logger.log(
          `Pergunta (Webhook) atualizada para respondida: ${externalId}`,
        );
      }
      return;
    }
  ...
  private async handleOrderV2(
    ...
    if (exists) {
      if (
        ...
        try {
          ...
          this.logger.log(
            `Auto-Mensagem pós-venda disparada para pedido atualizado: ${externalId}`,
          );
        } catch (e) {
          this.logger.error(
            `Falha ao enviar mensagem automática (update): ${(e as Error).message}`,
          );
        }
      }
      ...
      this.logger.log(
        `Pedido (Webhook) atualizado: ${externalId} com status ${order.status}`,
      );
      return;
    }
    ...
    if (order.status === 'paid' && order.pack_id) {
      try {
        ...
        this.logger.log(
          `Auto-Mensagem pós-venda disparada com sucesso para pedido novo: ${externalId}`,
        );
      } catch (e) {
        this.logger.error(
          `Falha ao enviar mensagem automática (novo): ${(e as Error).message}`,
        );
      }
    }
  }
  ...
  private async mlFetch(resource: string, accessToken: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `Falha ao buscar ${resource}: ${(err as Error).message}`,
      );
      return null;
    }
  }
